!function($){
  ...
  * 来源：http://blog.sina.com.cn/s/blog_8207314b0102wfmz.html
  * ================================= */
  ...
  , process: function (items) {
      var queryString=this.query;//输入框传入的参数
      var that = this;
      var filteritems=[];
      if (!items.length) {
        return this.shown ? this.hide() : this
      ...
      //author by lsx
      var field=this.options.fieldlable;
      if(field!=''){
          for(var i=0;i<items.length;i++){
            if(items[i][field].indexOf(queryString)>=0){//存在
                filteritems.push(items[i]);
            }
          }
      }
      if(filteritems.length>0){
          return this.render(filteritems.slice(0, this.options.items)).show()
      }else{
